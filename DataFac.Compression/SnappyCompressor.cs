...
public sealed class SnappyCompressor : IBlobCompressor
{
    ...
    public static CompressResult CompressData(ReadOnlyMemory<byte> data, Span<byte> hashSpan, int maxEmbeddedSize)
    {
        ...
        if (data.Length < 1024)
        {
            ...
            if (Snappy.TryCompress(data.Span, outputSpan, out int bytesWritten))
            {
                return bytesWritten < data.Length
                    ? new CompressResult(data.Length, BlobHashAlgo.Sha256, BlobCompAlgo.Snappy, outputSpan.Slice(0, bytesWritten).ToArray())
                    : new CompressResult(data.Length, BlobHashAlgo.Sha256, BlobCompAlgo.UnComp, data);
            }
        }
        ...
        return compressedData.Length < data.Length
            ? new CompressResult(data.Length, BlobHashAlgo.Sha256, BlobCompAlgo.Snappy, compressedData.Compact())
            : new CompressResult(data.Length, BlobHashAlgo.Sha256, BlobCompAlgo.UnComp, data);
    }

    public static CompressResult CompressText(string text, Span<byte> hashSpan, int maxEmbeddedSize)
    {
        ...
        if (text.Length < 1024)
        {
            ...
            if (Encoding.UTF8.TryGetBytes(text, localBuffer, out int bytesEncoded))
            {
                ...
                {
                    return new CompressResult(bytesEncoded, BlobHashAlgo.None, BlobCompAlgo.UnComp, inputSpan.ToArray());
                }
                ...
                if (Snappy.TryCompress(inputSpan, outputSpan, out int bytesWritten))
                {
                    return bytesWritten < bytesEncoded
                        ? new CompressResult(bytesEncoded, BlobHashAlgo.Sha256, BlobCompAlgo.Snappy, outputSpan.Slice(0, bytesWritten).ToArray())
                        : new CompressResult(bytesEncoded, BlobHashAlgo.Sha256, BlobCompAlgo.UnComp, inputSpan.ToArray());
                }
            }
#else
            ...
            {
                return new CompressResult(inputMemory1.Length, BlobHashAlgo.None, BlobCompAlgo.UnComp, inputMemory1);
            }
            ...
            if (Snappy.TryCompress(inputMemory1.Span, outputSpan, out int bytesWritten))
            {
                return bytesWritten < inputMemory1.Length
                    ? new CompressResult(inputMemory1.Length, BlobHashAlgo.Sha256, BlobCompAlgo.Snappy, outputSpan.Slice(0, bytesWritten).ToArray())
                    : new CompressResult(inputMemory1.Length, BlobHashAlgo.Sha256, BlobCompAlgo.UnComp, inputMemory1);
            }
#endif
        }
        ...
        {
            return new CompressResult(inputMemory2.Length, BlobHashAlgo.None, BlobCompAlgo.UnComp, inputMemory2);
        }
        ...
        return compressedData.Length < inputMemory2.Length
            ? new CompressResult(inputMemory2.Length, BlobHashAlgo.Sha256, BlobCompAlgo.Snappy, compressedData)
            : new CompressResult(inputMemory2.Length, BlobHashAlgo.Sha256, BlobCompAlgo.UnComp, inputMemory2);
    }
}